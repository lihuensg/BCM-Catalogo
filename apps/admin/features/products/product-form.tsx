...
export function ProductForm({ initial }: {
    ...
    async function save(e: React.FormEvent) { e.preventDefault(); if (busy)
        ...
        if (!initial)
            window.location.replace('/productos/' + result.data.id + '/editar');
    }
    catch (e) {
 ...
 setErrors(fieldErrors(e));
    ...
    return <><PageHeader title={initial ? 'Editar producto' : 'Nuevo producto'} description={initial ? initial.name : 'Completá los datos y guardá un borrador o publicá el producto.'} actions={<Link className="button button-secondary" href="/productos">Volver a productos</Link>}/><Toast message={message}/><nav className="product-form-nav" aria-label="Secciones del producto"><a href="#producto-informacion">Información</a><a href="#producto-categoria">Categoría</a><a href="#producto-precio">Precio</a><a href="#producto-disponibilidad">Disponibilidad</a><a href="#producto-imagenes">Imágenes</a><a href="#producto-especificaciones">Especificaciones</a><a href="#producto-merchandising">Merchandising</a><a href="#producto-seo">SEO</a><a href="#producto-publicacion">Publicación</a></nav><form className="form-sections" onSubmit={save}><fieldset disabled={busy}><Card id="producto-informacion" title="Información básica"><div className="form-grid">{text('name', 'Nombre', 200, true)}{text('slug', 'Slug', 220, true)}{text('sku', 'SKU', 100)}<div className="full-width">{text('shortDescription', 'Descripción breve', 500, true, true)}</div><div className="full-width">{text('fullDescription', 'Descripción completa', 100000, false, true)}</div></div></Card><Card id="producto-categoria" title="Categoría y marca" description="Las especificaciones corresponden a la categoría directa del producto."><div className="form-grid"><div><RemoteSelect path="/admin/categories" label="Categoría" value={input.categoryId ?? ''} selectedLabel={categoryLabel} required onChange={(id, label) => { if (id === input.categoryId)
