...
async function login(page: Page) { const f = fixture(); await page.goto('/login'); await page.getByLabel('Email', { exact: true }).fill(f.email); await page.getByLabel('Contraseña', { exact: true }).fill(f.password); await page.getByRole('button', { name: 'Ingresar al panel' }).click(); await expect(page).toHaveURL(/\/$/); await expect(page.getByRole('heading', { name: 'Dashboard', exact: true })).toBeVisible(); }
...
test('login validates credentials, protects routes server-side and revokes logout', async ({ page }) => { await page.goto('/productos'); await expect(page).toHaveURL(/\/login$/); await page.getByLabel('Email', { exact: true }).fill('missing@example.test'); await page.getByLabel('Contraseña', { exact: true }).fill('invalid'); await page.getByRole('button', { name: 'Ingresar al panel' }).click(); await expect(page.locator('.notice-error')).toContainText('email o la contraseña'); await login(page); await page.goto('/login'); await expect(page).toHaveURL(/\/$/); await page.getByRole('button', { name: 'Salir', exact: true }).click(); await expect(page).toHaveURL(/\/login$/); await page.goto('/configuracion'); await expect(page).toHaveURL(/\/login$/); });
test('real product journey creates, validates, edits and deactivates with dynamic attributes and media', async ({ page }) => { const f = fixture(); await login(page); await page.goto('/productos/nuevo'); await page.getByLabel('Nombre', { exact: true }).fill('Producto QA con nombre largo para validar el diseño responsive'); await page.getByLabel('Slug', { exact: true }).fill(f.prefix + '-product'); await page.getByLabel('Descripción breve', { exact: true }).fill('Producto de prueba controlado por la suite de UI.'); await choose(page, 'Categoría', f.categoryId); await page.getByLabel('Precio actual', { exact: true }).fill('1234567890123456.78'); await page.getByRole('switch', { name: 'Producto activo / publicado' }).check(); await page.getByRole('button', { name: 'Guardar producto' }).click(); await expect(page.getByText('Completá todas las especificaciones requeridas para publicar.')).toBeVisible(); await page.getByLabel('Especificación QA · Requerido al publicar', { exact: true }).fill('Valor QA'); await page.getByRole('button', { name: '+ Agregar imagen', exact: true }).click(); await page.getByLabel('URL de imagen 1', { exact: true }).fill('http://localhost:3101/logo.jpg'); await page.getByLabel('Texto alternativo 1', { exact: true }).fill('Imagen BCM de prueba'); await page.getByLabel('Imagen principal', { exact: true }).check(); await page.getByRole('button', { name: 'Guardar producto' }).click(); await expect(page).toHaveURL(/\/productos\/[^/]+\/editar$/); await page.getByLabel('Nombre', { exact: true }).fill('Producto QA editado'); await page.getByRole('button', { name: 'Guardar producto' }).click(); await expect(page.getByRole('status').filter({ hasText: 'Producto guardado correctamente.' })).toBeVisible(); await page.goto('/productos?search=' + f.prefix); await expect(page.getByRole('link', { name: 'Producto QA editado' })).toBeVisible(); const productRow = page.getByRole('row').filter({ hasText: 'Producto QA editado' }); await productRow.getByRole('button', { name: 'Desactivar', exact: true }).click(); await page.getByRole('dialog').getByRole('button', { name: 'Desactivar', exact: true }).click(); await expect(page.getByRole('status').filter({ hasText: 'Producto desactivado.' })).toBeVisible(); await expect(page.getByRole('row').filter({ hasText: 'Producto QA editado' }).getByRole('cell', { name: 'Inactivo', exact: true })).toBeVisible(); });
test('master forms persist categories, brands, options, associations and banners', async ({ page }) => {
    ...
    await page.getByLabel('Imagen desktop URL', { exact: true }).fill('http://localhost:3101/logo.jpg');
    await page.getByRole('dialog').getByRole('button', { name: 'Guardar', exact: true }).click();
...
test('API outage offers retry and empty results preserve navigation', async ({ page }) => { await login(page); await page.route('**/api/admin-proxy/admin/products?**', route => route.abort()); await page.goto('/productos'); await expect(page.locator('.notice-error')).toContainText('No pudimos conectar'); await page.unroute('**/api/admin-proxy/admin/products?**'); await page.getByRole('button', { name: 'Volver a intentar' }).click(); await expect(page.locator('.notice-error')).toHaveCount(0); await page.goto('/productos?search=no-such-product-' + fixture().prefix); await expect(page.getByRole('heading', { name: 'No encontramos productos' })).toBeVisible(); });
